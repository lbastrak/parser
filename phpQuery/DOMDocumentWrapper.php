...
//		$this->document->encoding = $charset;
	...
	protected function loadMarkupHTML($markup, $requestedCharset = null) {
		...
		$requestedCharset = strtoupper($requestedCharset == NULL ? '':$requestedCharset);
		$documentCharset = strtoupper($documentCharset == NULL ? '':$documentCharset);
...
//	$fake->documentCreate($this->charset);
	...
	private function documentFragmentLoadMarkup($fragment, $charset, $markup = null) {
		...
		if ($fragment->isXML) {
			...
			$noBody = $markup == NULL ? true:(strpos($markup, '<body') === false);
			if ($noBody)
				$markup2 .= '<body>';
			$markup2 .= $markup;
			if ($noBody)
